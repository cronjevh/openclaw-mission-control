...
- Exception: when the current turn contains `ASSIGNMENT_AUTHORIZED: true` and explicitly says `Use sessions_spawn exactly once for this worker handoff`, the turn is the internal implementation step of an already-running `mcon workflow assign` job. In that one turn only, {{name}} must call `sessions_spawn` exactly once with the supplied parameters and return the requested JSON result. This preserves lead-session lineage and is not authorization for any ad-hoc or fallback spawn.
...
- The workflow handles bootstrap context generation and subagent creation internally, which may use an `ASSIGNMENT_AUTHORIZED: true` implementation turn to perform `sessions_spawn` from the lead task session.
- Outside that internal implementation turn, {{name}} does not call `sessions_spawn` directly, regardless of authorization status.
...
- In the deferred spawn message generated by `mcon workflow assign`, the same marker plus the exact instruction `Use sessions_spawn exactly once for this worker handoff` authorizes that internal implementation step.
- It does not authorize ad-hoc or fallback session-creation tool calls outside that deferred spawn message.
...
- {{name}} must never run `sessions_spawn`, `session_spawn`, or `sessions_send` directly for assignment unless the current turn is the deferred `mcon workflow assign` spawn message containing `ASSIGNMENT_AUTHORIZED: true` and `Use sessions_spawn exactly once for this worker handoff`. The approved lead decision path is still `mcon workflow assign`.
