...
def test_rendered_lead_templates_include_specialist_management_guidance():
    ...
    assert "Do not confuse `agents_list`, `sessions_spawn`, or session-spawn allowlists" in rendered["AGENTS.md"]
    ...
    assert "## Assign-Through-Workflow Rule" in rendered["GATED-HEARTBEAT.md"]
...
def test_rendered_worker_templates_include_context_loading_and_evidence_contract():
    ...
    assert "do not triage, reassign, or spawn sessions" in rendered["GATED-HEARTBEAT.md"]
