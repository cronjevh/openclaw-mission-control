...
@"
You are a strict sysadmin script code-review judge.
...
function Test-StaticScriptContract {
    ...
    if ($ScriptText -notmatch '(?i)(--check|--dry-run)\b') {
        $issues += "Missing --check or --dry-run argument handling"
    }
    if ($ScriptText -notmatch '(?i)(--apply)\b') {
        $issues += "Missing --apply argument handling"
    }

    return $issues
}

function Test-StaticSafetyChecks {
    param([Parameter(Mandatory = $true)][string]$ScriptText)

    $issues = @()

    if ($ScriptText -notmatch 'set\s+-euo\s+pipefail') {
        $issues += "Missing 'set -euo pipefail'"
    }
    if ($ScriptText -match "(?i)(password|secret|token|api_key)\s*=\s*['\""][^'\"""]+['\"""]") {
        $issues += "Possible hardcoded secret detected"
    }
    if ($ScriptText -match '(?i)rm\s+-rf\s+/') {
        $issues += "Dangerous pattern detected: rm -rf /"
    }

    return $issues
}

function Test-StaticSudoAudit {
    param([Parameter(Mandatory = $true)][string]$ScriptText)

    $issues = @()

    $sudoMatches = [regex]::Matches($ScriptText, '(?i)sudo\s+(\S+)')
    foreach ($m in $sudoMatches) {
        $cmd = $m.Groups[1].Value
        if ($cmd -match '^(tee|cat|less|more|grep|awk|sed|echo|printf|ls|pwd|whoami|id|test|true|false|stat|file)$') {
            $issues += "Unnecessary sudo for read-only command: $cmd"
        }
    }
...
function Test-StaticEnvironmentAwareness {
    param([Parameter(Mandatory = $true)][string]$ScriptText)

    $issues = @()

    if ($ScriptText -match '(?i)ufw|firewall') {
        $issues += "Script references firewall/ufw - ensure WSL2 compatibility (no host-level firewall rules)"
    }
...
$scriptPathAbs = Resolve-AbsolutePath -Path $ScriptPath
...
$scriptText = Get-Content -LiteralPath $scriptPathAbs -Raw
$judgeSpec = Get-Content -LiteralPath $judgeSpecPathAbs -Raw | ConvertFrom-Json -Depth 100

$timestamp = (Get-Date).ToUniversalTime().ToString("yyyyMMddTHHmmssZ")
$staticResultPath = Join-Path $effectiveEvidenceDir ("validation-static-{0}-{1}.json" -f $TaskId, $timestamp)
$finalResultPath = Join-Path $effectiveEvidenceDir ("validation-result-{0}.json" -f $TaskId)

# Run static checks first
$staticContractIssues = Test-StaticScriptContract -ScriptText $scriptText
$staticSafetyIssues = Test-StaticSafetyChecks -ScriptText $scriptText
$staticSudoIssues = Test-StaticSudoAudit -ScriptText $scriptText
$staticEnvIssues = Test-StaticEnvironmentAwareness -ScriptText $scriptText

$staticPassed = ($staticContractIssues.Count -eq 0) -and ($staticSafetyIssues.Count -eq 0) -and ($staticSudoIssues.Count -eq 0)
$staticResult = [pscustomobject]@{
    task_id = $TaskId
    check_kind = "static_analysis"
    script_path = $scriptPathAbs
    contract_passed = ($staticContractIssues.Count -eq 0)
    contract_issues = $staticContractIssues
    safety_passed = ($staticSafetyIssues.Count -eq 0)
    safety_issues = $staticSafetyIssues
    sudo_passed = ($staticSudoIssues.Count -eq 0)
    sudo_issues = $staticSudoIssues
    env_passed = ($staticEnvIssues.Count -eq 0)
    env_issues = $staticEnvIssues
    overall_passed = $staticPassed
}
Write-TextFile -Path $staticResultPath -Content (ConvertTo-PrettyJson -InputObject $staticResult)

if (-not $staticPassed) {
    $result = [pscustomobject]@{
        task_id = $TaskId
        validator_kind = "sysadmin_static"
        ...
        static_result_path = $staticResultPath
        passed = $false
        failure_reason = "Static analysis failed"
...
# Static checks passed; proceed to LLM judge
$prompt = New-JudgePrompt -JudgeSpec $judgeSpec -ScriptText $scriptText -ScriptPath $scriptPathAbs -TaskId $TaskId
...
$result = [pscustomobject]@{
    ...
    model = $Model
    target_artifact_path = $scriptPathAbs
